...
{
	try {
		// Obtain the text of the dropped item
		var urlString = event.dataTransfer.getData("text/plain");	
		
		// Shorten the URL
		newUrlString = urlString.replace(/^(http:\/\/[^\/]+)\/.*(\/dp\/[^\/]+)\/.*$/, "$1$2");

		// Set the text field to the new value
		var textFieldToChange = document.getElementById("textfield");
		textFieldToChange.value = newUrlString;
...
// drag events so I can handle the drop when it occurs
...
function copyToClipboard(event) {
	...
	var dataForClipboard = textFieldValue.value;
	
	var script = "/usr/bin/osascript -e 'set the clipboard to \"" + dataForClipboard + "\"'";
    widget.system(script, null);
}

function openUrlInBrowser(event) {
	var textFieldValue = document.getElementById("textfield");
	var url = textFieldValue.value;
